...
import { AICHATURL, AICOVERURL } from '../../../constant/Constant';
...
const useChatApi = () => {
  ...
  const sendTextMessageToApi = async (
    ...
    try {
      ...
      if (text.includes('翻唱歌曲')) {
        const response = await axios.post(`${AICOVERURL}/api/chat`, { message: text, session_id: sessionId });
        ...
        if (response.data.audioUrl) {
          ...
          const base64Data = response.data.audioUrl.replace('data:audio/wav;base64,', '');
          // 将 base64 转换为 Blob
          const audioBlob = new Blob([Buffer.from(base64Data, 'base64')], { type: 'audio/wav' });
        ...
        // 创建 EventSource 连接
        //不断地更新响应内容
        const eventSource = new EventSource(`${AICHATURL}/api/chat?message=${encodeURIComponent(text)}&session_id=${sessionId}`);
        let fullContent = '';
  ...
  const uploadFileToApi = async (file: File, addMessage: (message: Message) => void) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${AICOVERURL}/api/upload`, formData, {
